 ...
 *
 * FIX 1: Instantiation for uut_mult corrected.
 * FIX 2: Robust handshake logic re-implemented.
 */
`timescale 1ns / 1ps
...
module kalman_fsm_3d #(
    ...
    reg [3:0] load_count;
    reg [3:0] save_count; // For addsub wait
    
    // --- Instantiate "Tool-belt" ---
    
    // **FIX 1**: Module name is 'matrix_mult_3x3', it does not take
    // ACC_WIDTH as a parameter, and ports are 'c_row'/'c_col'.
    matrix_mult_3x3 #(
        .M(M), .N(N), .P(P), 
        ...
        .c_row(mult_row), .c_col(mult_col) 
    );
    
    // **FIX 1.b**: Assuming matrix_add_sub_3x3 is the fixed version
    matrix_add_sub_3x3 #(
        .M(M), .P(P), 
        .DATA_WIDTH(DATA_WIDTH)
    ) uut_addsub (
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            addsub_a_wen <= 0;
            addsub_b_wen <= 0;
            // ** FIX 2: done is controlled by IDLE/DONE states **
            
            case (state)
                STATE_IDLE: begin
                    // ** FIX 2: Robust handshake logic **
                    done <= 0; 
                    if (start) begin
                ...
                STATE_PRED_4_WAIT_SAVE: begin
                    if (mult_c_valid) begin
                        ...
                        // ** Note: Verilog-2001 uses '>>>' for arithmetic shift **
